...
\title{基于热风干燥的圆柱形中药材传热传质耦合模型与数值仿真}
\author{}
\date{}
...
$\alpha$ & 海底坡度角（$^\circ$） \\
$\theta$ & 多波束换能器开角（$^\circ$） \\
$\gamma$ & 半开角，$\gamma = \theta/2$（$^\circ$） \\
$W$ & 条带覆盖宽度（m） \\
$d$ & 相邻两条测线的水平间距（m） \\
$\eta$ & 相邻条带重叠率 \\
$\sigma$ & 两条覆盖带在坡面上的有符号重叠长度（m） \\
$\beta$ & 测线方向与坡面法向水平投影的夹角（$^\circ$） \\
$\psi$ & 全局测线航向角（$^\circ$） \\
$q$ & 分位数松弛参数 \\
$L_{\text{total}}$ & 测线总长度（m） \\
$P_{\text{miss}}$ & 漏测海区面积百分比 \\
\bottomrule
\end{tabular}
\caption*{主要符号说明}
\end{table}
...
\subsection{问题一的分析}
预热平衡阶段，热量由烘房空气传至药材表面并向中心传导，水分由内部向表面扩散并向环境蒸发，二者共同决定温度与水分浓度演化。药材为长 \(25~\mathrm{cm}\)、半径 \(2~\mathrm{cm}\) 的圆柱，长径比远大于 \(1\)，端面影响可忽略，故简化为以中心轴为对称轴、固定半径的一维径向传热传质问题。热量传递遵循热传导规律，水分传递遵循扩散规律，扩散系数 \(D=7\times10^{-9}e^{-0.89/C}\) 随水分浓度变化。问题一中热物性为常数，温度场与水分场无直接双向耦合，故分开求解。
中心取对称边界；表面与热空气直接接触，热量与水分均采用 Robin 边界条件，烘房空气温度与水分浓度由附件 1 插值得到，作为时变驱动量。初始温度 \(28~^\circ\mathrm{C}\)，初始水分浓度 \(2.55~\mathrm{kg/kg}\)。
空间采用有限体积法，划分同心环形控制体，写出“储存量变化＝内侧通量－外侧通量”的守恒收支；水分方程经 Kirchhoff 变换构造界面非线性通量，得到稀疏常微分方程组后，时间方向用自适应隐式 BDF 推进。温度方程以圆柱 Robin 边界的 Bessel--Duhamel 级数解作独立基准，并做网格与时间容差收敛检查，最终求得 \(1800~\mathrm{s}\) 内温度与水分浓度的时空分布，并输出指定结果至 \texttt{result1.xlsx}。
...
\begin{equation}
\rho c_p\frac{\partial T}{\partial t}=\frac{1}{r}\frac{\partial}{\partial r}\left(rk\frac{\partial T}{\partial r}\right),\quad 0<r<R,
\label{eq:q1heat}
\end{equation}
水分浓度场满足非线性扩散方程
\begin{equation}
\frac{\partial C}{\partial t}=\frac{1}{r}\frac{\partial}{\partial r}\left(rD(C)\frac{\partial C}{\partial r}\right),\quad 0<r<R,
\label{eq:q1moisture}
\end{equation}
其中
\begin{equation}
D(C)=7\times10^{-9}e^{-0.89/C}.
\label{eq:q1diffusivity}
\end{equation}
