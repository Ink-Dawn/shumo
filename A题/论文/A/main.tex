...
\item 第四问中，药材长度保持不变，各材料层沿径向同比例收缩，作为温湿计算的外部输入。
...
\begin{figure}[H]
\centering
\qoneplaceholder{3.2cm}{圆柱整体与轴向中截面 \(\longrightarrow\) 径向微元\\
标出 \(R,L,r,\mathrm{d}r\) 及热量、水分的传递方向}
...
为观察变化如何由表面传向内部，我们将各时刻的径向分布绘于图~\ref{fig:q1field}。再用初始物性比较两种过程的快慢：热扩散系数 \(\alpha=k/(\rho c_p)=1.69\times10^{-7}~\mathrm{m^2/s}\)，水分扩散系数 \(D(C_0)=4.94\times10^{-9}~\mathrm{m^2/s}\)，对应的特征时间分别为 \(R^2/\alpha\approx39.5~\mathrm{min}\) 和 \(R^2/D(C_0)\approx22.5~\mathrm h\)。后者约为前者的 \(34.2\) 倍，与计算中升温先于明显失水传入内部的现象一致。这里用初始扩散系数估计时间尺度，实际失水过程仍按变化的 \(D(C)\) 计算。
  ...
  \begin{figure}[H]
      ...
      \caption{预热阶段温湿时空分布与末态中截面；圆形截面由径向解作轴对称映射得到}
      \label{fig:q1field}
  \end{figure}
...
\end{equation}
其中 \(\operatorname{erfc}\) 为余误差函数。这一上界与二维网格无关，结合对照结果，我们在第一问的时段内忽略端面对中截面的影响。图~\ref{fig:q1-endface} 中的差异主要集中在端部；计入端面交换后，\(1800~\mathrm{s}\) 的整根平均含水率较对照降低约 \(0.01860~\mathrm{kg/kg}\)。因此，上述简化仅用于本问中截面的径向分布，不能据此忽略整根平均状态或端部的变化。

\begin{figure}[H]
\centering
\qoneplaceholder{3.0cm}{半圆柱纵截面位置条与端部局部放大\\[0.5em]
温度、含水率的二维--同网格一维最大差分布}
\caption{端面影响的空间分布；各点取 \(0:10:1800~\mathrm{s}\) 保存时刻上的最大绝对差}
\label{fig:q1-endface}
\end{figure}
...
\caption{第二问数值求解检验}
\label{tab:q2-checks}
\begin{tabularx}{\linewidth}{|>{\centering\arraybackslash}p{0.16\linewidth}|>{\centering\arraybackslash}X|>{\centering\arraybackslash}p{0.22\linewidth}|>{\centering\arraybackslash}p{0.22\linewidth}|}
\hline
